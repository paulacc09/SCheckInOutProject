...
const googleNoConfig = (req, res) =>
  res.status(503).json({
    ok: false,
    message: 'Login con Google no está configurado. Define GOOGLE_CLIENT_ID y GOOGLE_CLIENT_SECRET en .env',
  });
...
if (passport.googleOAuthConfigured) {
  ...
  router.get(
    ...
    (req, res) => {
      ...
      res.redirect((process.env.FRONTEND_URL || 'http://localhost:5173') + '/auth/google?token=' + token);
    }
  );
} else {
